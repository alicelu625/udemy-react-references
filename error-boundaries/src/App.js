...
class App extends Component {
  ...
  render() { //Reach calls this method to render something to screen
    let persons = null;
    let btnClass = [classes.Button]; //array of Button classes
    ...
    if (this.state.showPersons) {
      persons = (
        <div>
          ...
          {this.state.persons.map((person, index) => { {/*takes element of array*/}
            return <ErrorBoundary key={person.id}> {/*key has to be outer element*/}
              <Person
      ...
      btnClass.push(classes.Red); //change dynamically
    }
    ...
    return (
      <div className={classes.App}>
        ...
        {/*alt as a prop to pass to StyledButton*/}
